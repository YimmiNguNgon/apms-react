    ...
    <section style={{ background: '#fff', border: '1px solid #dbe3ee', borderRadius: 6, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap', background: '#f7f9fc' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {editable && !editMode && (
            <button type="button" onClick={startEdit} disabled={!report || saving} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: 'none', borderRadius: 4, padding: '6px 10px', background: '#2563eb', color: '#fff', fontWeight: 700, cursor: 'pointer' }}><Pencil size={14} />Chỉnh sửa</button>
          )}
          {editable && editMode && (
            <>
